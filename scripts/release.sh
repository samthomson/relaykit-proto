...
# Keep the changelog in sync: the release entry must be the newest one in the file.
DATE=$(date +%F)
CHANGELOG_ENTRY="## $VERSION — $DATE"
if ! head -3 app/CHANGELOG.md | grep -qF "$CHANGELOG_ENTRY"; then
  if $CHECK_ONLY; then
    fail "app/CHANGELOG.md has no entry for $VERSION ($CHANGELOG_ENTRY) — add it or run the full release"
  fi
  # Prepend the entry right after the "# changelog" heading.
  TMP=$(mktemp)
  printf '# changelog\n\n%s\n%s\n' "$CHANGELOG_ENTRY" "$NOTES" > "$TMP"
  tail -n +2 app/CHANGELOG.md >> "$TMP"
  mv "$TMP" app/CHANGELOG.md
...
git add app/version.json app/release.yml app/CHANGELOG.md
